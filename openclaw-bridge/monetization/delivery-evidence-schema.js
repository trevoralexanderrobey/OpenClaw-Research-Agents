...
const PHASE28_EXPORT_EVENTS_SCHEMA = "phase28-export-events-v1";
const PHASE28_EXPORT_EVENT_SCHEMA = "phase28-export-event-v1";
const PHASE28_EVIDENCE_LEDGER_SCHEMA = "phase28-delivery-evidence-ledger-v1";
const PHASE28_EVIDENCE_EVENT_SCHEMA = "phase28-delivery-evidence-event-v1";
const PHASE28_EVIDENCE_SNAPSHOT_SCHEMA = "phase28-delivery-evidence-snapshot-v1";
const PHASE28_EVIDENCE_INDEX_SCHEMA = "phase28-delivery-evidence-index-v1";
const PHASE28_VERIFY_STATUS_SCHEMA = "phase28-delivery-evidence-verification-status-v1";

const EXPORT_EVENT_TYPES = Object.freeze(["bundle_exported"]);
const EVIDENCE_EVENT_TYPES = Object.freeze(["delivery_outcome_recorded", "delivery_correction_recorded"]);
...
const TERMINAL_DELIVERY_STATES = Object.freeze(["delivery_completed", "withdrawn"]);
...
function validateSha(value, code, message) {
  ...
  if (!/^[a-f0-9]{64}$/.test(normalized)) {
    ...
    error.code = safeString(code) || "PHASE28_SHA_INVALID";
    throw error;
  }
  return normalized;
}
...
function ensureAllowed(value, allowedValues, code, message) {
  ...
  if (!allowedValues.includes(normalized)) {
    ...
    error.code = safeString(code) || "PHASE28_VALUE_INVALID";
    throw error;
  }
  return normalized;
}
...
function normalizeIso(value, code, fieldName) {
  ...
  if (!normalized || !Number.isFinite(Date.parse(normalized))) {
    ...
    error.code = safeString(code) || "PHASE28_TIME_INVALID";
    throw error;
  }
  return normalized;
}

function normalizeRelativePath(inputPath, code = "PHASE28_PATH_INVALID") {
...
function ensureFileType(value, code, message) {
  ...
  if (!ALLOWED_EVIDENCE_EXTENSIONS.includes(normalized)) {
    ...
    error.code = safeString(code) || "PHASE28_FILE_TYPE_INVALID";
    throw error;
  }
  return normalized;
}
...
function normalizeExportArtifactRefs(input) {
  const refs = asArray(input).map((entry) => {
    ...
    if (byteSize < 0) {
      ...
      error.code = "PHASE28_EXPORT_ARTIFACT_BYTE_SIZE_INVALID";
      throw error;
    }
    const artifactPath = normalizeRelativePath(item.path || item.export_path || item.file, "PHASE28_EXPORT_ARTIFACT_PATH_INVALID");
    const fileType = safeString(item.file_type || normalizeFileExtension(artifactPath) || "folder").toLowerCase();
    return canonicalize({
      byte_size: byteSize,
      file_type: fileType,
      path: artifactPath,
      sha256: validateSha(item.sha256, "PHASE28_EXPORT_ARTIFACT_SHA_INVALID", "export artifact sha256 must be valid")
    });
  });
  ...
  if (sorted.length !== unique.length) {
    ...
    error.code = "PHASE28_EXPORT_ARTIFACT_PATH_DUPLICATE";
    throw error;
  }
  return sorted;
}

function normalizeExportedDeliveryTargets(value) {
  const targets = sortUnique(asStringArray(value));
  if (targets.length === 0) {
    const error = new Error("exported_delivery_targets is required");
    error.code = "PHASE28_EXPORT_TARGETS_REQUIRED";
    throw error;
  }
  return targets;
}

function normalizeEvidenceStoredPath(inputPath, deliveryTarget, code = "PHASE28_EVIDENCE_STORED_PATH_INVALID") {
  const target = safeString(deliveryTarget);
  if (!target) {
    const error = new Error("delivery_target is required");
    error.code = "PHASE28_EVIDENCE_DELIVERY_TARGET_REQUIRED";
    throw error;
  }
  const normalized = normalizeRelativePath(inputPath, code);
  const expectedPrefix = `delivery-evidence/${target}/evidence/`;
...
function normalizeEvidenceRefs(input, deliveryTarget) {
  const refs = asArray(input).map((entry) => {
    const item = asPlainObject(entry);
    const byteSize = asInt(item.byte_size, -1);
    if (byteSize < 0) {
      const error = new Error("evidence ref byte_size must be >= 0");
      error.code = "PHASE28_EVIDENCE_REF_BYTE_SIZE_INVALID";
      throw error;
    }
    const originalFileName = safeString(item.original_filename);
    if (!originalFileName) {
      const error = new Error("evidence ref original_filename is required");
      error.code = "PHASE28_EVIDENCE_REF_ORIGINAL_FILENAME_REQUIRED";
      throw error;
    }
    const storedPath = normalizeEvidenceStoredPath(item.stored_path, deliveryTarget, "PHASE28_EVIDENCE_REF_STORED_PATH_INVALID");
    const fileType = ensureFileType(
      item.file_type || normalizeFileExtension(originalFileName) || normalizeFileExtension(storedPath),
      "PHASE28_EVIDENCE_REF_FILE_TYPE_INVALID",
    ...
    return canonicalize({
      ...
      sha256: validateSha(item.sha256, "PHASE28_EVIDENCE_REF_SHA_INVALID", "evidence ref sha256 must be valid"),
  ...
  if (sorted.length !== unique.length) {
    ...
    error.code = "PHASE28_EVIDENCE_REF_PATH_DUPLICATE";
    throw error;
  }
  return sorted;
}
...
function normalizeStateTransition(value) {
  ...
  return canonicalize({
    from: ensureAllowed(
      transition.from,
      DELIVERY_STATES,
      "PHASE28_EVIDENCE_STATE_FROM_INVALID",
      "state_transition.from is invalid"
    ),
    to: ensureAllowed(
      transition.to,
      DELIVERY_STATES,
      "PHASE28_EVIDENCE_STATE_TO_INVALID",
...
function assertNonEmptyPayload(payload = {}) {
  ...
  if (evidenceRefs.length < 1 && !externalRef && !notes) {
    ...
    error.code = "PHASE28_EVIDENCE_PAYLOAD_EMPTY";
    throw error;
  }
}
...
function buildExportEventBase(input = {}) {
  ...
  if (!offerId) {
    ...
    error.code = "PHASE28_EXPORT_OFFER_ID_REQUIRED";
    throw error;
  }
  const operatorId = safeString(source.operator_id);
  if (!operatorId) {
    const error = new Error("operator_id is required");
    error.code = "PHASE28_EXPORT_OPERATOR_ID_REQUIRED";
    throw error;
  }
  return canonicalize({
    schema_version: PHASE28_EXPORT_EVENT_SCHEMA,
    event_type: ensureAllowed(
      source.event_type,
      EXPORT_EVENT_TYPES,
      "PHASE28_EXPORT_EVENT_TYPE_INVALID",
      "export event_type is invalid"
    ),
    offer_id: offerId,
    approved_bundle_hash: validateSha(source.approved_bundle_hash, "PHASE28_EXPORT_APPROVED_HASH_INVALID", "approved_bundle_hash must be valid"),
    exported_at: normalizeIso(source.exported_at, "PHASE28_EXPORT_TIME_INVALID", "exported_at"),
    export_format: ensureAllowed(source.export_format, ["folder", "zip"], "PHASE28_EXPORT_FORMAT_INVALID", "export_format must be folder or zip"),
    operator_id: operatorId,
    exported_delivery_targets: normalizeExportedDeliveryTargets(source.exported_delivery_targets),
...
function buildEvidenceEventBase(input = {}) {
  ...
  if (!offerId) {
    const error = new Error("offer_id is required");
    error.code = "PHASE28_EVIDENCE_OFFER_ID_REQUIRED";
    throw error;
  }
  const deliveryTarget = safeString(source.delivery_target);
  if (!deliveryTarget) {
    const error = new Error("delivery_target is required");
    error.code = "PHASE28_EVIDENCE_DELIVERY_TARGET_REQUIRED";
    throw error;
  ...
  if (!operatorId) {
    const error = new Error("operator_id is required");
    error.code = "PHASE28_EVIDENCE_OPERATOR_ID_REQUIRED";
    throw error;
  }
  const idempotencyKey = safeString(source.idempotency_key);
  if (!idempotencyKey) {
    const error = new Error("idempotency_key is required");
    error.code = "PHASE28_IDEMPOTENCY_KEY_REQUIRED";
    throw error;
  }
  const base = canonicalize({
    schema_version: PHASE28_EVIDENCE_EVENT_SCHEMA,
    recorded_at: normalizeIso(source.recorded_at, "PHASE28_EVIDENCE_RECORDED_AT_INVALID", "recorded_at"),
    event_type: ensureAllowed(
      source.event_type,
      EVIDENCE_EVENT_TYPES,
      "PHASE28_EVIDENCE_EVENT_TYPE_INVALID",
    ...
    approved_bundle_hash: validateSha(source.approved_bundle_hash, "PHASE28_EVIDENCE_APPROVED_HASH_INVALID", "approved_bundle_hash must be valid"),
    external_ref: safeString(source.external_ref),
    notes: safeString(source.notes),
    evidence_refs: normalizeEvidenceRefs(source.evidence_refs, deliveryTarget),
...
function computeExportEventHash(eventWithoutEventHash = {}) {
  return sha256(`${PHASE28_EXPORT_EVENT_SCHEMA}|${JSON.stringify(canonicalize(eventWithoutEventHash))}`);
}

function computeEvidenceEventHash(eventWithoutEventHash = {}) {
  return sha256(`${PHASE28_EVIDENCE_EVENT_SCHEMA}|${JSON.stringify(canonicalize(eventWithoutEventHash))}`);
}
...
function attachExportEventHashes(baseEvent, sequence, prevEventHash) {
  ...
  if (normalizedSequence < 1) {
    ...
    error.code = "PHASE28_EXPORT_SEQUENCE_INVALID";
    throw error;
  }
  const prevHash = validateSha(prevEventHash || ZERO_HASH, "PHASE28_EXPORT_PREV_HASH_INVALID", "prev_event_hash must be valid");
...
function attachEvidenceEventHashes(baseEvent, sequence, prevEventHash) {
  ...
  if (normalizedSequence < 1) {
    const error = new Error("sequence must be >= 1");
    error.code = "PHASE28_EVIDENCE_SEQUENCE_INVALID";
    throw error;
  }
  const prevHash = validateSha(prevEventHash || ZERO_HASH, "PHASE28_EVIDENCE_PREV_HASH_INVALID", "prev_event_hash must be valid");
  const payload = canonicalize({
...
function validateExportEvent(eventInput = {}) {
  ...
  if (safeString(event.schema_version) !== PHASE28_EXPORT_EVENT_SCHEMA) {
    const error = new Error(`export event schema_version must be ${PHASE28_EXPORT_EVENT_SCHEMA}`);
    error.code = "PHASE28_EXPORT_SCHEMA_INVALID";
    throw error;
  }
  ...
  const prevEventHash = validateSha(event.prev_event_hash, "PHASE28_EXPORT_PREV_HASH_INVALID", "export prev_event_hash must be valid");
  const base = canonicalize({
  ...
  const eventHash = validateSha(event.event_hash, "PHASE28_EXPORT_EVENT_HASH_INVALID", "export event_hash must be valid");
  if (eventHash !== expectedHash) {
    const error = new Error("export event_hash mismatch");
    error.code = "PHASE28_EXPORT_EVENT_HASH_MISMATCH";
    throw error;
  }
  return canonicalize({
    ...base,
...
function validateEvidenceEvent(eventInput = {}) {
  const event = asPlainObject(eventInput);
  if (safeString(event.schema_version) !== PHASE28_EVIDENCE_EVENT_SCHEMA) {
    const error = new Error(`evidence event schema_version must be ${PHASE28_EVIDENCE_EVENT_SCHEMA}`);
    error.code = "PHASE28_EVIDENCE_SCHEMA_INVALID";
    throw error;
  }
  const sequence = asInt(event.sequence, 0);
  if (sequence < 1) {
  ...
  const prevEventHash = validateSha(event.prev_event_hash, "PHASE28_EVIDENCE_PREV_HASH_INVALID", "evidence prev_event_hash must be valid");
  const base = canonicalize({
  ...
  const eventHash = validateSha(event.event_hash, "PHASE28_EVIDENCE_EVENT_HASH_INVALID", "evidence event_hash must be valid");
  if (eventHash !== expectedHash) {
    const error = new Error("evidence event_hash mismatch");
    error.code = "PHASE28_EVIDENCE_EVENT_HASH_MISMATCH";
    throw error;
  }
  return canonicalize({
    ...base,
...
function createEmptyExportEventsStore() {
  return canonicalize({
    schema_version: PHASE28_EXPORT_EVENTS_SCHEMA,
    next_sequence: 0,
    chain_head: "",
    events: []
  });
...
function createEmptyEvidenceLedgerStore() {
  return canonicalize({
    schema_version: PHASE28_EVIDENCE_LEDGER_SCHEMA,
    next_sequence: 0,
    chain_head: "",
    events: []
  });
...
function validateExportEventsStore(storeInput = {}) {
  const shape = validateStoreShape(storeInput, PHASE28_EXPORT_EVENTS_SCHEMA, "PHASE28_EXPORT_STORE");
  const chain = verifyEventChain(asPlainObject(storeInput).events, validateExportEvent, "PHASE28_EXPORT_STORE", "export events");

  if (shape.chain_head !== chain.chain_head) {
    const error = new Error("export store chain_head mismatch");
    error.code = "PHASE28_EXPORT_STORE_CHAIN_HEAD_MISMATCH";
    throw error;
  }
  if (shape.next_sequence !== chain.next_sequence) {
    const error = new Error("export store next_sequence mismatch");
    error.code = "PHASE28_EXPORT_STORE_NEXT_SEQUENCE_MISMATCH";
    throw error;
  }
...
function validateEvidenceLedgerStore(storeInput = {}) {
  const shape = validateStoreShape(storeInput, PHASE28_EVIDENCE_LEDGER_SCHEMA, "PHASE28_EVIDENCE_STORE");
  const chain = verifyEventChain(asPlainObject(storeInput).events, validateEvidenceEvent, "PHASE28_EVIDENCE_STORE", "evidence events");

  if (shape.chain_head !== chain.chain_head) {
    const error = new Error("evidence store chain_head mismatch");
    error.code = "PHASE28_EVIDENCE_STORE_CHAIN_HEAD_MISMATCH";
    throw error;
  }
  if (shape.next_sequence !== chain.next_sequence) {
    const error = new Error("evidence store next_sequence mismatch");
    error.code = "PHASE28_EVIDENCE_STORE_NEXT_SEQUENCE_MISMATCH";
    throw error;
  }
...
function buildEvidenceIdempotencyComparable(input = {}) {
  ...
  const evidenceRefs = asArray(source.evidence_refs).map((entry) => {
    ...
    return canonicalize({
      ...
      sha256: validateSha(item.sha256, "PHASE28_EVIDENCE_REF_SHA_INVALID", "evidence_ref sha256 must be valid")
    });
  }).sort((left, right) => {
  ...
  return canonicalize({
    ...
    delivery_target: safeString(source.delivery_target),
...
module.exports = {
  ...
  PHASE28_EVIDENCE_INDEX_SCHEMA,
  PHASE28_EVIDENCE_LEDGER_SCHEMA,
  PHASE28_EVIDENCE_SNAPSHOT_SCHEMA,
