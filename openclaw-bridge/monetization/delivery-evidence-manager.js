...
} = require("./manual-delivery-state-machine.js");
const { createDeliveryEvidenceLedger } = require("./delivery-evidence-ledger.js");
...
async function hashFileMetadata(filePath) {
  ...
  if (!stat.isFile()) {
    ...
    error.code = "PHASE28_EVIDENCE_FILE_TYPE_INVALID";
    throw error;
  }
...
function computeDirectoryHash(directoryPath) {
  ...
  return crypto.createHash("sha256").update(`phase28-export-folder-v1|${JSON.stringify(canonicalize(files))}`).digest("hex");
}
...
function buildExistingComparable(event) {
  return buildEvidenceIdempotencyComparable({
    ...
    delivery_target: safeString(event.delivery_target),
...
function createDeliveryEvidenceManager(options = {}) {
  ...
  if (!releaseApprovalManager || typeof releaseApprovalManager.validateApprovedRelease !== "function") {
    const error = new Error("delivery evidence manager requires releaseApprovalManager.validateApprovedRelease");
    error.code = "PHASE28_APPROVAL_MANAGER_REQUIRED";
    throw error;
  }
  ...
  const ledger = createDeliveryEvidenceLedger({
    rootDir,
  ...
  function resolveApprovalContext(offerId) {
    ...
    const approvedBundleHash = validateSha(
      ...
      "PHASE28_APPROVED_BUNDLE_HASH_INVALID",
      "release-approval hash_of_release_bundle must be valid"
    );
    const offerPath = path.join(validated.bundle_dir, "offer.json");
    const offer = asPlainObject(JSON.parse(fs.readFileSync(offerPath, "utf8")));
    const approvedTargets = asArray(offer.direct_delivery_targets)
      .map((entry) => safeString(entry))
      .filter(Boolean)
    ...
    return {
      ...
      approved_delivery_targets: approvedTargets,
      offer_id: safeString(approval.offer_id) || safeString(offerId)
    };
  }

  function ensureApprovedTarget(approvedTargets, deliveryTarget) {
    const target = safeString(deliveryTarget);
    if (!approvedTargets.includes(target)) {
      const error = new Error(`delivery_target '${target}' is not approved for this bundle`);
      error.code = "PHASE28_DELIVERY_TARGET_NOT_APPROVED";
      throw error;
    }
    return target;
  }
  ...
  function ensureExportCoverage(exportStore, target, approvedBundleHash) {
    ...
    if (coverage.length < 1) {
      const error = new Error(`delivery_target '${target}' is not export-eligible`);
      error.code = "PHASE28_EXPORT_ELIGIBILITY_REQUIRED";
      throw error;
    }
    return coverage;
  }
  ...
  async function collectIncomingEvidenceMeta(files) {
    ...
    for (const filePath of files) {
      ...
      if (!ALLOWED_EVIDENCE_EXTENSIONS.includes(extension)) {
        ...
        error.code = "PHASE28_EVIDENCE_FILE_EXTENSION_INVALID";
        throw error;
      }
...
function ensureEventPayloadNotEmpty(incomingEvidenceRefs, externalRef, notes) {
  if (incomingEvidenceRefs.length < 1 && !safeString(externalRef) && !safeString(notes)) {
    const error = new Error("delivery outcome requires at least one payload element: evidence files, external_ref, or notes");
  ...
  function ensureAttachmentQuotas(incomingEvidenceRefs) {
    if (incomingEvidenceRefs.length > quotas.max_files_per_event) {
      ...
      error.code = "PHASE28_EVENT_FILE_COUNT_EXCEEDED";
      throw error;
    }
    for (const ref of incomingEvidenceRefs) {
      if (Number(ref.byte_size || 0) > quotas.max_bytes_per_file) {
        ...
        error.code = "PHASE28_FILE_SIZE_EXCEEDED";
        throw error;
      }
    }
  }
  ...
  function buildIncomingComparable(input = {}) {
    ...
    return buildEvidenceIdempotencyComparable({
      ...
      delivery_target: source.delivery_target,
  ...
  async function stageAttachments(input = {}) {
    ...
    ensurePathInside(offerDir, evidenceDir, "PHASE28_EVIDENCE_PATH_ESCAPE");
    ...
    try {
      for (let index = 0; index < incomingRefs.length; index += 1) {
        ...
        const storedPath = ledger.buildStoredEvidencePath(deliveryTarget, fileName);
        const absoluteTarget = path.join(offerDir, normalizeRelativePath(storedPath).split("/").join(path.sep));
        ensurePathInside(offerDir, absoluteTarget, "PHASE28_EVIDENCE_PATH_ESCAPE");
        ...
        if (copied.sha256 !== ref.sha256 || copied.byte_size !== Number(ref.byte_size || 0)) {
          ...
          error.code = "PHASE28_EVIDENCE_STAGING_HASH_MISMATCH";
          throw error;
        }
      ...
      if (error && !error.code) {
        error.code = "PHASE28_ATTACHMENT_STAGING_FAILED";
      }
      throw error;
    }
  ...
  async function recordExportEvent(input = {}) {
    ...
    const exportedTargets = asArray(source.exported_delivery_targets || source.exportedDeliveryTargets)
      .map((entry) => safeString(entry))
      .filter(Boolean)
      .sort((left, right) => left.localeCompare(right));
    ...
    for (const target of exportedTargets) {
      ensureApprovedTarget(approvalContext.approved_delivery_targets, target);
    }

    const eventBase = buildExportEventBase({
      schema_version: "phase28-export-event-v1",
      ...
      exported_delivery_targets: exportedTargets,
  ...
  async function recordDeliveryOutcome(input = {}) {
    const source = asPlainObject(input);
    const offerId = safeString(source.offer_id || source.offerId);
    const operatorId = safeString(source.operator_id || source.operatorId);
    const deliveryTarget = safeString(source.delivery_target || source.deliveryTarget);
    ...
    const eventType = safeString(source.event_type || source.eventType || "delivery_outcome_recorded");

    if (!offerId) {
      const error = new Error("offer_id is required");
      error.code = "PHASE28_OFFER_ID_REQUIRED";
      throw error;
    }
    if (!operatorId) {
      const error = new Error("operator_id is required");
      error.code = "PHASE28_OPERATOR_ID_REQUIRED";
      throw error;
    }
    if (!deliveryTarget) {
    ...
    if (!outcomeState) {
      ...
      error.code = "PHASE28_OUTCOME_STATE_REQUIRED";
      throw error;
    }
    if (!idempotencyKey) {
    ...
    if (!EVIDENCE_EVENT_TYPES.includes(eventType)) {
      ...
      error.code = "PHASE28_EVIDENCE_EVENT_TYPE_INVALID";
      throw error;
    }
    ...
    const approvalContext = resolveApprovalContext(offerId);
    ensureApprovedTarget(approvalContext.approved_delivery_targets, deliveryTarget);
    ...
    return ledger.withOfferLock(offerId, async () => {
      ...
      ensureExportCoverage(exportStore, deliveryTarget, approvalContext.approved_bundle_hash);
      ...
      if (existing) {
        ...
        if (!compareIdempotencyPayload(existingComparable, comparableInput)) {
          ...
          error.code = "PHASE28_IDEMPOTENCY_CONFLICT";
          throw error;
        }
        return canonicalize({
          ok: true,
      ...
      const fromState = current.current_state || READY_FOR_MANUAL_DELIVERY;
      ...
      if (evidenceStore.events.length >= quotas.max_total_evidence_events_per_offer) {
        ...
        error.code = "PHASE28_OFFER_EVENT_QUOTA_EXCEEDED";
        throw error;
      }
      ...
      if (existingEvidenceBytes + incomingBytes > quotas.max_total_evidence_bytes_per_offer) {
        ...
        error.code = "PHASE28_OFFER_EVIDENCE_BYTES_QUOTA_EXCEEDED";
        throw error;
      }

      const nextSequence = Number(evidenceStore.next_sequence || 0) + 1;
      const offerDir = ledger.getOfferDir(offerId);
      const evidenceDir = ledger.getDeliveryTargetEvidenceDirPath(offerId, deliveryTarget);
      ...
      const baseEvent = buildEvidenceEventBase({
        schema_version: "phase28-delivery-evidence-event-v1",
  ...
  async function verifyOfferDeliveryEvidence(input = {}) {
    const source = asPlainObject(input);
    const offerId = safeString(source.offer_id || source.offerId);
    ...
    if (
      mode === "incremental"
      && safeString(priorStatus.schema_version) === PHASE28_VERIFY_STATUS_SCHEMA
    ...
    for (const event of exportStore.events) {
      if (safeString(event.offer_id) !== offerId) {
        ...
        error.code = "PHASE28_EXPORT_EVENT_OFFER_MISMATCH";
        throw error;
      }
      if (safeString(event.approved_bundle_hash) !== approvalContext.approved_bundle_hash) {
        ...
        error.code = "PHASE28_EXPORT_EVENT_APPROVED_HASH_MISMATCH";
        throw error;
      }
      for (const target of asArray(event.exported_delivery_targets)) {
        ensureApprovedTarget(approvalContext.approved_delivery_targets, target);
      }
    }
    ...
    for (const event of evidenceStore.events) {
      if (safeString(event.offer_id) !== offerId) {
        ...
        error.code = "PHASE28_EVIDENCE_EVENT_OFFER_MISMATCH";
        throw error;
      }
      if (safeString(event.approved_bundle_hash) !== approvalContext.approved_bundle_hash) {
        const error = new Error("evidence event approved_bundle_hash does not match release approval");
        error.code = "PHASE28_EVIDENCE_EVENT_APPROVED_HASH_MISMATCH";
        throw error;
      }
      ensureApprovedTarget(approvalContext.approved_delivery_targets, event.delivery_target);
      ensureExportCoverage(exportStore, event.delivery_target, approvalContext.approved_bundle_hash);
    }

    for (const target of approvalContext.approved_delivery_targets) {
      deriveCurrentStateForTarget({
    ...
    for (const event of evidenceStore.events) {
      for (const ref of asArray(event.evidence_refs)) {
        const normalizedStoredPath = normalizeRelativePath(ref.stored_path, "PHASE28_EVIDENCE_REF_STORED_PATH_INVALID");
        const absolutePath = path.join(offerDir, normalizedStoredPath.split("/").join(path.sep));
        ensurePathInside(offerDir, absolutePath, "PHASE28_EVIDENCE_PATH_ESCAPE");
        if (!fs.existsSync(absolutePath)) {
          const error = new Error(`referenced evidence attachment is missing: ${normalizedStoredPath}`);
          error.code = "PHASE28_EVIDENCE_REF_MISSING";
          throw error;
        }
        ...
        if (safeString(ref.sha256) !== digest.sha256 || Number(ref.byte_size || 0) !== digest.byte_size) {
          ...
          error.code = "PHASE28_EVIDENCE_REF_HASH_MISMATCH";
          throw error;
        }
      }
    }
    ...
    const status = canonicalize({
      schema_version: PHASE28_VERIFY_STATUS_SCHEMA,
  ...
  async function rebuildDerivedForOffer(offerId) {
    ...
    const states = deriveStatesForTargets({
      delivery_targets: context.approved_delivery_targets,
      export_events_store: exportStore,
      evidence_ledger_store: evidenceStore
    });
    ...
    for (const state of states) {
      const deliveryTarget = safeString(state.delivery_target);
      const snapshot = canonicalize({
        schema_version: PHASE28_EVIDENCE_SNAPSHOT_SCHEMA,
        ...
        current_state: state.eligible === true ? safeString(state.current_state || READY_FOR_MANUAL_DELIVERY) : "",
        ...
        authoritative_sources: canonicalize({
          export_events: "delivery-evidence/export-events.json",
          evidence_ledger: "delivery-evidence/ledger.json"
        })
      });
      const snapshotPath = ledger.getDeliveryTargetSnapshotPath(offerId, deliveryTarget);
  ...
  async function rebuildRepoIndex() {
    ...
    for (const offerId of offerIds) {
      const offerPath = path.join(ledger.getOfferDir(offerId), "offer.json");
      ...
      try {
        ...
        offer = JSON.parse(fs.readFileSync(offerPath, "utf8"));
      } catch {
        continue;
      }

      const approvedBundleHash = safeString(approval.hash_of_release_bundle);
      const approvedTargets = asArray(offer && offer.direct_delivery_targets)
        .map((entry) => safeString(entry))
        .filter(Boolean)
        .sort((left, right) => left.localeCompare(right));
      ...
      const evidenceStore = ledger.readEvidenceLedgerStore(offerId);
      const states = deriveStatesForTargets({
        delivery_targets: approvedTargets,
        export_events_store: exportStore,
        evidence_ledger_store: evidenceStore
      });
      ...
      offers.push(canonicalize({
        ...
        evidence_event_count: evidenceStore.events.length,
        evidence_chain_head: safeString(evidenceStore.chain_head),
        delivery_targets: states.map((state) => canonicalize({
    ...
    for (const offer of offers) {
      offer.delivery_targets.sort((left, right) => left.delivery_target.localeCompare(right.delivery_target));
    }

    const indexBody = canonicalize({
      schema_version: PHASE28_EVIDENCE_INDEX_SCHEMA,
      generated_at: timeProvider.nowIso(),
      authoritative_source: "workspace/releases/<offerId>/delivery-evidence/export-events.json + ledger.json",
      offers
    });
  ...
  async function verifyAllOffers(input = {}) {
    ...
    for (const offerId of offerIds) {
      results.push(await verifyOfferDeliveryEvidence({
        offer_id: offerId,
        mode,
  ...
  async function buildExportArtifactRef(input = {}) {
    ...
    if (!exportPath || !fs.existsSync(exportPath)) {
      ...
      error.code = "PHASE28_EXPORT_ARTIFACT_MISSING";
      throw error;
    }
    ...
    if (!relPath || relPath.startsWith("..")) {
      ...
      error.code = "PHASE28_EXPORT_ARTIFACT_PATH_INVALID";
      throw error;
    }
  ...
  return Object.freeze({
    ...
    verifyAllOffers,
    verifyOfferDeliveryEvidence
  });
}

module.exports = {
  createDeliveryEvidenceManager,
