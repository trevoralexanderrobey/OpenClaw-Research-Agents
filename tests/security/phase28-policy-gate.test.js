...
const scriptPath = path.join(root, "scripts", "verify-phase28-policy.sh");
...
const REQUIRED_FIXTURE_FILES = [
  ...
  "docs/phase28-direct-delivery-governance.md",
  ...
  "scripts/verify-phase28-policy.sh",
  "package.json"
];

async function createFixture() {
  const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), "openclaw-phase28-policy-"));
...
test("phase28 policy gate passes for valid repository layout", async () => {
  const fixture = await createFixture();
  const run = runPolicy(fixture);
  assert.equal(run.status, 0, run.stderr || run.stdout);
  assert.match(run.stdout, /Phase 28 direct-delivery policy verification passed/);
});

test("phase28 policy gate fails when phase28 script wiring is removed from package.json", async () => {
  const fixture = await createFixture();
  const file = path.join(fixture, "package.json");
  const packageJson = JSON.parse(fs.readFileSync(file, "utf8"));
  delete packageJson.scripts["phase28:verify"];
  fs.writeFileSync(file, `${JSON.stringify(packageJson, null, 2)}\n`, "utf8");

  const run = runPolicy(fixture, { PHASE28_POLICY_FORCE_NO_RG: "1" });
  assert.notEqual(run.status, 0);
  assert.match(run.stderr, /phase28:verify/);
});

test("phase28 policy gate fails when unsafe network logic is introduced into phase28 paths", async () => {
  const fixture = await createFixture();
  const file = path.join(fixture, "openclaw-bridge", "monetization", "delivery-evidence-manager.js");
