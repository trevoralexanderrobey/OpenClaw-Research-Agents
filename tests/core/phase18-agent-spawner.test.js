...
function createMovingTimeProvider() {
  return {
...
test("phase18 spawn orchestration dispatches independent subtasks with bounded lane concurrency", async () => {
  ...
  const statusJson = JSON.parse(await fsp.readFile(path.join(missionBase, result.mission_id, "status.json"), "utf8"));
  assert.ok(Array.isArray(statusJson.lane_scaling_events));
  assert.ok(statusJson.lane_scaling_events.length >= 1);
  assert.ok(statusJson.lane_scaling_events.some((entry) => Number(entry.next_inflight || 0) > Number(entry.previous_inflight || 0)));
});

test("phase18 timeout handling pauses mission and preserves open loops for resume", async () => {
  const root = await makeRoot();
  const missionBase = path.join(root, "workspace", "missions");
  const outputDir = path.join(root, "workspace", "research-output");
  const runtimeStatePath = path.join(root, "state", "runtime", "state.json");
  ...
  const outputManager = createResearchOutputManager({
    outputDir,
    timeProvider: createMovingTimeProvider()
  });
  const laneQueue = createLaneQueue({
    persistencePath: path.join(root, "workspace", "comms", "events", "lane-queue.json"),
    timeProvider: createMovingTimeProvider()
  ...
  const autonomyLadder = createAutonomyLadder({ policyPath: autonomyPath });
  const roleRouter = createRoleRouter({ registry: agentRegistry, autonomyLadder });
  agentRegistry.registerRole("scout", async () => new Promise(() => {}));
    ...
    missionTemplates: {
      templates: {
        academic_trend_scan: {
          ...
          spawned_roles: ["scout"],
          steps: [
            { role: "scout", action_type: "collect_sources", task_type: "extract", timeout_ms: 30, input_strategy: "mission_inputs" }
          ]
        }
      }
    },
  ...
  await assert.rejects(
    ...
    (error) => error && error.code === "PHASE18_SUBTASK_TIMEOUT"
  );

  const statusJson = JSON.parse(await fsp.readFile(path.join(missionBase, missionEnvelope.mission_id, "status.json"), "utf8"));
  const runtimeState = await loadRuntimeState({ path: runtimeStatePath });
  assert.equal(statusJson.status, "paused");
  assert.equal(statusJson.resume_ready, true);
  assert.ok(Array.isArray(runtimeState.state.openLoops));
  assert.ok(runtimeState.state.openLoops.length >= 1);
});

test("phase18 stall detection pauses mission when queued work stops making progress", async () => {
  const root = await makeRoot();
  const missionBase = path.join(root, "workspace", "missions");
  const outputDir = path.join(root, "workspace", "research-output");
  const runtimeStatePath = path.join(root, "state", "runtime", "state.json");
  ...
  const agentRegistry = createAgentRegistry({
    topologyConfig: { roles: ["orchestrator", "scout", "analyst"] }
  });
  const autonomyLadder = createAutonomyLadder({ policyPath: autonomyPath });
  const roleRouter = createRoleRouter({ registry: agentRegistry, autonomyLadder });
  agentRegistry.registerRole("scout", async () => new Promise(() => {}));
  agentRegistry.registerRole("analyst", async () => ({ status: "completed" }));
    ...
    missionTemplates: {
      templates: {
        academic_trend_scan: {
          ...
          enabled: true,
          safety_class: "research_only",
          spawned_roles: ["scout", "analyst"],
          steps: [
            { role: "scout", action_type: "collect_sources", task_type: "extract", timeout_ms: 0, input_strategy: "mission_inputs" },
            { role: "analyst", action_type: "analyze_documents", task_type: "analyze", timeout_ms: 0, input_strategy: "mission_and_dependency_outputs", depends_on: [1] }
          ]
        }
      }
    },
    agentTopology: { roles: ["orchestrator", "scout", "analyst"] },
    skillProvider
  });
  const orchestrator = createSpawnOrchestrator({
    laneQueue,
  ...
  const missionInput = {
    template_id: "academic_trend_scan",
    description: "Stall mission",
    inputs: [{ path: inputPath, type: "path" }],
    constraints: {
      default_subtask_timeout_ms: 0,
      mission_max_runtime_ms: 0,
      stall_interval_ms: 40,
  ...
  await assert.rejects(
    ...
    (error) => error && error.code === "PHASE18_MISSION_STALLED"
  );
...
test("phase18 checkpoint synthesis writes deterministic intermediate summaries", async () => {
  const root = await makeRoot();
  const missionBase = path.join(root, "workspace", "missions");
  const outputDir = path.join(root, "workspace", "research-output");
  const runtimeStatePath = path.join(root, "state", "runtime", "state.json");
  ...
  const result = await spawner.spawnMission({
    template_id: "academic_trend_scan",
    description: "Checkpoint mission",
    inputs: [{ path: inputPath, type: "path" }],
    created_at: "2026-03-06T00:00:00.000Z"
  }, {
    supervisorDecision: { approved: true, decision_id: "sup-1" },
  ...
  const checkpointDir = path.join(missionBase, result.mission_id, "artifacts", "checkpoints");
  const checkpoints = (await fsp.readdir(checkpointDir)).sort();
  assert.ok(checkpoints.includes("checkpoint-stage-0001.json"));
  assert.ok(checkpoints.includes("checkpoint-threshold-0002.json"));
  const checkpointBody = JSON.parse(await fsp.readFile(path.join(checkpointDir, checkpoints[0]), "utf8"));
  assert.ok(Array.isArray(checkpointBody.completed_results));
  assert.ok(checkpointBody.completed_results.length >= 1);
});
