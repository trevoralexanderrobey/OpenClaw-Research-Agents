...
Phase 11 extends this runbook with recovery assurance and continuity controls:
- deterministic recovery checkpoints and backup manifest integrity verification
- operator-gated restore orchestration (`--approval-token`, `--scope governance.recovery.restore`, `--confirm`)
- advisory-only continuity SLO breach reporting and deterministic tabletop recovery drills
- failover readiness validation that never triggers autonomous failover

See `docs/phase11-recovery-assurance.md` for full Phase 11 operator procedures and policy boundaries.
