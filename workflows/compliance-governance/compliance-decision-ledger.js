...
function normalizeDecisionForHash(decision = {}) {
  return {
    ...
    targetRef: safeString(decision.targetRef),
    targetSha: safeString(decision.targetSha).toLowerCase(),
    ...
    asOfIso: safeString(decision.asOfIso),
    policySnapshotHash: safeString(decision.policySnapshotHash).toLowerCase()
  };
}

function verifyReleaseDecisionHashChain(decisionsInput) {
  const decisions = asArray(decisionsInput)
    .map((entry) => ReleaseGateDecisionRecordSchema.parse(entry))
  ...
  for (const decision of decisions) {
    if (Number(decision.sequence) <= previousSequence) {
      throw makeError("PHASE8_RELEASE_SEQUENCE_INVALID", "Release decision sequence must be strictly increasing", {
        sequence: decision.sequence,
        previousSequence
      });
    }

    const withoutHash = ReleaseGateDecisionRecordWithoutHashSchema.parse(normalizeDecisionForHash(decision));
    const expectedDecisionHash = computeReleaseDecisionHash(withoutHash);

    if (safeString(decision.prevDecisionHash) !== expectedPrevDecisionHash) {
      throw makeError("PHASE8_DECISION_PREV_HASH_MISMATCH", "Release decision prevDecisionHash mismatch", {
        sequence: decision.sequence,
    ...
    if (safeString(decision.decisionHash) !== expectedDecisionHash) {
      throw makeError("PHASE8_DECISION_HASH_MISMATCH", "Release decision hash mismatch", {
        sequence: decision.sequence,
...
function buildExpectedComplianceLedgerFromDecisions(decisionsInput) {
  const verified = verifyReleaseDecisionHashChain(decisionsInput);
  let previousChainHash = "";

  const records = verified.decisions.map((decision, index) => {
    const sequence = index + 1;
    const entry = ComplianceDecisionLedgerRecordSchema.parse({
      sequence,
    ...
    if (sequence > 1 && safeString(entry.prevDecisionHash) !== safeString(verified.decisions[index - 1].decisionHash)) {
      throw makeError("PHASE8_LEDGER_DECISION_CHAIN_INVALID", "Ledger decision chain link mismatch", {
        sequence,
...
function verifyComplianceDecisionLedgerChain(recordsInput, chainHeadInput = "") {
  const records = asArray(recordsInput)
    .map((entry) => ComplianceDecisionLedgerRecordSchema.parse(entry))
    .sort((left, right) => Number(left.sequence || 0) - Number(right.sequence || 0));
  ...
  for (const record of records) {
    if (Number(record.sequence) !== previousSequence + 1) {
      throw makeError("PHASE8_LEDGER_SEQUENCE_INVALID", "Ledger sequence must be contiguous starting at 1", {
    ...
    if (safeString(record.chainHash) !== expectedChainHash) {
      throw makeError("PHASE8_LEDGER_CHAIN_HASH_MISMATCH", "Ledger chain hash mismatch", {
        sequence: record.sequence,
        expectedChainHash,
  ...
  if (chainHead !== lastChainHash) {
    throw makeError("PHASE8_LEDGER_CHAIN_HEAD_MISMATCH", "Compliance decision ledger chain head mismatch", {
...
function verifyComplianceDecisionIntegrity(state) {
  const governance = state && state.complianceGovernance && typeof state.complianceGovernance === "object"
    ? state.complianceGovernance
    : null;
  if (!governance) {
    throw makeError("PHASE8_LEDGER_STATE_INVALID", "complianceGovernance block is required");
  }

  const decisions = asArray(governance.releaseGates);
  ...
  const expected = buildExpectedComplianceLedgerFromDecisions(decisions);
  const actual = verifyComplianceDecisionLedgerChain(asArray(ledger.records), safeString(ledger.chainHead));

  if (actual.count !== expected.records.length) {
    throw makeError("PHASE8_LEDGER_LENGTH_MISMATCH", "Compliance ledger length does not match release decisions", {
  ...
  for (let index = 0; index < expected.records.length; index += 1) {
    ...
    if (
      ...
      throw makeError("PHASE8_LEDGER_RECORD_MISMATCH", "Compliance ledger record differs from deterministic projection", {
  ...
  if (actualNextSequence < expectedNextSequence) {
    throw makeError("PHASE8_LEDGER_NEXT_SEQUENCE_INVALID", "Compliance ledger nextSequence is behind observed records", {
...
function repairTruncatedComplianceLedgerTail(state) {
  const governance = state && state.complianceGovernance && typeof state.complianceGovernance === "object"
    ? state.complianceGovernance
    : null;
  if (!governance) {
  ...
  if (actualRecords.length > expected.records.length) {
    throw makeError("PHASE8_LEDGER_REPAIR_NOT_TRUNCATED", "Compliance ledger has extra records; truncated-tail repair is not allowed");
  }

  for (let index = 0; index < actualRecords.length; index += 1) {
    const actual = ComplianceDecisionLedgerRecordSchema.parse(actualRecords[index]);
    ...
    if (
      ...
      throw makeError("PHASE8_LEDGER_REPAIR_NOT_TRUNCATED", "Compliance ledger diverges before tail; truncated-tail repair is not allowed", {
...
module.exports = {
  verifyReleaseDecisionHashChain,
  buildExpectedComplianceLedgerFromDecisions,
  verifyComplianceDecisionLedgerChain,
