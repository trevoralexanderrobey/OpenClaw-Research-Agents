...
function parseArgs(argv) {
  const out = {
    ...
    notes: "",
    evidenceFiles: [],
    eventType: "delivery_outcome_recorded",
    confirm: false,
    unknown: []
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === "--delivery-target") { out.deliveryTarget = String(argv[index + 1] || "").trim(); index += 1; continue; }
...
function usage() {
  return [
    "Usage:",
    "  node scripts/record-delivery-outcome.js --offer-id <offer_id> --delivery-target <delivery_target>",
...
async function main() {
  ...
  if (!args.confirm) {
    process.stderr.write("Delivery outcome recording rejected: --confirm is required\n");
    process.exit(1);
  }

  const runtime = buildMonetizationRuntime();
  const result = await runtime.deliveryEvidenceManager.recordDeliveryOutcome({
    offer_id: args.offerId,
    delivery_target: args.deliveryTarget,
