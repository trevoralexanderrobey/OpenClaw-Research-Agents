...
async function main() {
  ...
  if (args.unknown.length > 0 || missing.length > 0) {
    ...
    process.stderr.write("Usage: node scripts/rotate-token.js --token-id <id> --confirm\n");
    process.exit(1);
  }

  const result = await runtime.tokenManager.rotateToken(args.tokenId, {
    role: "operator",
    requester: args.operatorId,
    confirm: true,
    correlationId: `phase13-rotate-token-${args.operatorId}`
  });
